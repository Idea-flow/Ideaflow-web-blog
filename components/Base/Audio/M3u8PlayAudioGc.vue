...
<template>
  <div class="bg-gray-100 p-3 sm:p-4 rounded-lg flex flex-row gap-3 sm:gap-4">
    <!-- 左侧封面和播放控制区域 -->
    <div class="relative w-1/3 flex items-center justify-center">
      <div class="relative w-full aspect-square max-w-[180px] sm:max-w-xs">
        ...
        >
          ...
          <svg 
            v-if="!isPlaying" 
            class="w-12 h-12 sm:w-16 sm:h-16 text-white opacity-80 drop-shadow-lg" 
            fill="currentColor" 
            viewBox="0 0 24 24"
          >
          ...
          <!-- 暂停图标（播放时显示在右下角） -->
          <div v-else class="absolute bottom-1.5 right-1.5 sm:bottom-2 sm:right-2 bg-black bg-opacity-50 rounded-full p-1">
    ...
    <div class="flex-1 flex flex-col justify-between">
      ...
      <div class="mb-3 sm:mb-4">
        <h3 class="text-lg sm:text-xl font-bold text-gray-800 truncate">{{ audio.title }}</h3>
      </div>
      
      <!-- 进度条和时间信息 -->
      <div class="space-y-2">
        <div 
          class="progress-bar w-full h-1.5 sm:h-2 bg-gray-300 rounded-full cursor-pointer relative"
          @click="handleClick"
          @mousedown="startDrag"
        >
          <!-- 缓冲进度条容器 -->
          ...
          <!-- 播放进度条 -->
          <div 
            class="relative h-full bg-blue-600 rounded-full transition-all duration-300 ease-out" 
            :style="{ width: progressPercentage + '%' }"
          >
            <!-- 拖拽点 -->
            <div class="absolute right-0 top-1/2 transform translate-x-1/2 -translate-y-1/2 w-3 h-3 sm:w-4 sm:h-4 bg-blue-600 rounded-full opacity-0 hover:opacity-100 transition-opacity duration-200 shadow-md"></div>
          </div>
        </div>
      ...
      <div class="mt-3 sm:mt-4 flex items-center space-x-2 sm:space-x-3 relative">
        ...
        <div class="relative"
          ...
          <div 
            ...
            class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 bg-white p-2 rounded-lg shadow-lg w-32"
          >
            <input
              type="range"
              v-model="volume"
              ...
              orient="vertical"
            >
            <div class="text-center text-xs text-gray-600 mt-1">{{ Math.round(volume * 100) }}%</div>
          </div>
        </div>
        ...
        <div class="relative"
          ...
          <button 
            ...
            class="px-2 py-1 rounded hover:bg-gray-200 focus:outline-none text-sm sm:text-base"
          >
            {{ playbackRate }}x
          </button>
          ...
          >
            <div class="grid grid-cols-2 gap-1">
              <button
                ...
                class="px-2 py-1 text-sm rounded hover:bg-blue-100"
                :class="{ 'bg-blue-500 text-white': playbackRate === rate }"
              >
                {{ rate }}x
              </button>
            </div>
