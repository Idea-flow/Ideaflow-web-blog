      ...
      <template v-for="(message, index) in messages" :key="message.id">
        ...
        <div v-else :class="['flex items-start', message.isSelf ? 'flex-row-reverse' : '']">
          ...
          <div :class="['max-w-[45%]', message.isSelf ? 'flex flex-col items-end' : 'flex flex-col items-start']">
            ...
            <!-- 图片消息 -->
            <div v-else-if="message.type === 1" class="max-w-[240px] rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-200">
              <img :src="message.content" 
                   class="w-full h-auto object-cover rounded-lg cursor-pointer hover:scale-[1.02] transition-transform duration-200" 
                   loading="lazy"
                   @load="scrollToBottom"
                   alt="图片消息">
            </div>
