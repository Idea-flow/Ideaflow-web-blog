...
import { getFilePresignedUrl } from '~/services/file'
...
const activeTab = ref('local') // 当前激活的tab
...
<template>
  <div class="relative">
    ...
    <div
      ...
      ref="panelRef"
      class="absolute bottom-full left-0 mb-2 w-72 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-lg shadow-xl p-4 border border-gray-100 dark:border-gray-700 transform transition-all duration-300 ease-in-out"
    >
      <!-- Tab切换 -->
      <div class="flex mb-4 border-b border-gray-200 dark:border-gray-700">
        <button
          ...
          :class="{
            'text-blue-500 border-b-2 border-blue-500': activeTab === 'local',
            'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300': activeTab !== 'local'
          }"
        ...
        <button
          class="flex-1 py-2 text-sm font-medium transition-colors"
          :class="{
            'text-blue-500 border-b-2 border-blue-500': activeTab === 'url',
            'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300': activeTab !== 'url'
          }"
          @click="activeTab = 'url'"
        >
          网络图片
      ...
      <div v-if="previewImage" class="mb-4">
        <img :src="previewImage" class="w-full h-40 object-contain rounded-lg border border-gray-200 dark:border-gray-700">
      </div>

      <!-- 本地上传区域 -->
      <div v-if="activeTab === 'local'">
        <div
          class="border-2 border-dashed rounded-lg p-4 mb-4 text-center cursor-pointer transition-colors"
          :class="{
        ...
        <button
          ...
          @click="() => { emit('select', previewImage, 1); showUploadPanel = false; previewImage = '' }"
        >
          确认
        </button>
      </div>
      ...
      <div v-if="activeTab === 'url'" class="space-y-2">
        <input
          ...
          type="text"
          placeholder="输入图片URL"
          class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200"
        >
        <button
        ...
        <button
          v-if="previewImage"
          class="w-full px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors"
          @click="() => { emit('select', previewImage, 1); showUploadPanel = false; previewImage = ''; imageUrl = '' }"
        >
          确认
        </button>
      </div>
