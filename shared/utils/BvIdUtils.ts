const BASE = [
    '3', 'Z', 'b', '8', 'F', 'p', 'D', 'X',
    'm', 'W', '7', 'k', 'J', 'H', 's', 'Q',
    'y',  'v', 'N', 'd', 'T', 'R', '6', 'S',
    'u', '9', 'i', 'c', 'B', '4', 'E', 'g',
    'L', '5', 'z', 'j', 'O', '2', 'P', 'h',
    'M', 'e', 'f', 't', '1', 'V', 'r', 'I',
    'x', 'n', 'q', 'w', 'C', 'K', 'G', 'Y',
    'l', 'U', 'o'
]

const SUFFIX_CHAR = 'A'
const BIN_LEN = BASE.length
const CODE_LEN = 8

export const idToCode = (id: number): string => {
    let buf: string[] = new Array(BIN_LEN)
    let charPos = BIN_LEN
    let num = id

    while (Math.floor(num / BIN_LEN) > 0) {
        const index = num % BIN_LEN
        buf[--charPos] = BASE[index]
        num = Math.floor(num / BIN_LEN)
    }

    buf[--charPos] = BASE[num % BIN_LEN]
    let result = buf.slice(charPos).join('')

    if (result.length < CODE_LEN) {
        const suffixChars: string[] = [SUFFIX_CHAR]
        for (let i = 0; i < CODE_LEN - result.length - 1; i++) {
            suffixChars.push(BASE[Math.floor(Math.random() * BIN_LEN)])
        }
        result += suffixChars.join('')
    }

    return `BV${result}`
}

export const codeToId = (code: string): number => {
    const cleanCode = code.split('BV')[1]
    const charArray = cleanCode.split('')
    let result = 0

    for (let i = 0; i < charArray.length; i++) {
        let index = 0
        for (let j = 0; j < BIN_LEN; j++) {
            if (charArray[i] === BASE[j]) {
                index = j
                break
            }
        }

        if (charArray[i] === SUFFIX_CHAR) {
            break
        }

        result = i > 0 ? result * BIN_LEN + index : index
    }

    return result
}
